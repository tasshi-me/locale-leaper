...
import {getLocaleOffline, getRequestToken, getTimezoneOffline, updateLocale, updateTimezone} from "./fetchers.ts";
...
export const useRequestToken: Hook<string, "requestToken"> = () => {
  ...
  const {data: requestToken} = useSWR("requestToken", ()=>getRequestToken(tabId), {suspense: true})
...
export const useLocale: HookWithMutate<Locale, "locale"> = ()=> {
  ...
  const {data: locale, mutate} = useSWR("locale", ()=> getLocaleOffline(tabId), {suspense: true});
...
export const useTimezone: HookWithMutate<Timezone, "timezone"> = ()=> {
  const {tabId} = useTabId();
  const {requestToken} = useRequestToken();
  const {data: timezone, mutate} = useSWR("timezone", ()=>getTimezoneOffline(tabId), {suspense: true});
