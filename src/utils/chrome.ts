...
export const getTabId = async () => {
  const tabs = await chrome.tabs.query({
    active: true, currentWindow: true, url: [
      "https://*.cybozu.com/k/*",
      "https://*.cybozu.com/g/*",
...
export const getEmbeddedInformation = async (tabId: number, key: EmbeddedInformationKey): Promise<string> => {
    ...
    func: async (_params): Promise<{ data: Data } | { error: string }> => {
      try {
        const resp = await fetch(_params.path, {
          ...
          body: _params.method === "GET" ? undefined : JSON.stringify({
