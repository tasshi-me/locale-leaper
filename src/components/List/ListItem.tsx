...
const Component = <Item, Key extends string>({className, item, createLabel, createKey,callback, current}: Props<Item, Key>): React.ReactNode=> {
  ...
  return (<li className={className} ref={current ? scrollBottomRef :undefined }><button onClick={()=> callback(key)}>{label}</button></li>)
}
