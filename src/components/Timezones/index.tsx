...
import {localeToTimezoneLabelId, Timezone, timezones} from "../../utils/timezone.ts";
...
const Timezones: React.FC<Props> = ({callback}) => {
  ...
  const createLabel = (t: typeof timezones[number]): React.ReactNode => (<><span>{t.utc}</span>
  <span>{t[localeToTimezoneLabelId(locale)]}</span></>
)
  const createKey = (t: typeof timezones[number]): Timezone => t.id
  return (<>
    <h2>Timezone</h2>
    <List items={timezones} createLabel={createLabel} createKey={createKey} initialActiveKey={timezone} maxHeight={"20em"} callback={async (key) => {
